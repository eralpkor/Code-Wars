...
Rules
1. The input string will always be lower
case but maybe empty.
...
function wave(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    // Regex check from a-z if only true then push it to result array.
    if (/[a-z]/.test(str[i])) result.push(str.slice(0, i) + str[i].toUpperCase() + str.slice(i + 1, str.length));
  }
